...
print(response.model)  # "gpt-4o-mini" if degraded
```
...
instrument(MeterOptions(
    api_key=os.environ["ADEN_API_KEY"],
    fail_open=True,  # Default: allow requests if server unreachable
))
```

## Local Development

For testing without a server connection, use `before_request` to mock budget logic:
...
<CardGroup cols={2}>
  <Card title="PydanticAI" icon="robot" href="/sdk/python/pydantic-ai">
    Type-safe agent framework integration
  </Card>
  <Card title="Frameworks" icon="cubes" href="/sdk/python/frameworks">
